...
def acquire_and_display_images(cam, nodemap, nodemap_tldevice):
    ...
    if CHOSEN_IR_TYPE == IRFormatType.LINEAR_10MK:
        ...
        node_temp_linear_high = PySpin.CEnumEntryPtr(
            node_IRFormat.GetEntryByName('TemperatureLinear10mK'))
        ...
        node_IRFormat = PySpin.CEnumerationPtr(nodemap.GetNode('IRFormat'))
        node_temp_linear_low = PySpin.CEnumEntryPtr(
            node_IRFormat.GetEntryByName('TemperatureLinear100mK'))
        ...
        node_IRFormat = PySpin.CEnumerationPtr(nodemap.GetNode('IRFormat'))
        node_temp_radiometric = PySpin.CEnumEntryPtr(
            node_IRFormat.GetEntryByName('Radiometric'))
    ...
    try:
        ...
        CalibrationQueryJ1_node = PySpin.CFloatPtr(
            nodemap.GetNode('J1'))    # Gain
        J1 = CalibrationQueryJ1_node.GetValue()
        print('Gain =', J1)

        CalibrationQueryJ0_node = PySpin.CIntegerPtr(
            nodemap.GetNode('J0'))   # Offset
        ...
        if CHOSEN_IR_TYPE == IRFormatType.RADIOMETRIC:
            ...
            H2O = Humidity * np.exp(1.5587 + 0.06939 * TAtmC - 0.00027816 *
                                    TAtmC * TAtmC + 0.00000068455 * TAtmC * TAtmC * TAtmC)
            print('H20 =', H2O)

            Tau = X * np.exp(-np.sqrt(Dist) * (A1 + B1 * np.sqrt(H2O))) + \
                (1 - X) * np.exp(-np.sqrt(Dist) * (A2 + B2 * np.sqrt(H2O)))
            print('tau =', Tau)
            ...
            r3 = ((1 - ExtOpticsTransmission) / (Emiss * Tau *
                  ExtOpticsTransmission)) * (R / (np.exp(B / ExtOpticsTemp) - F))
            print('r3 =', r3)
            ...
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    if CHOSEN_IR_TYPE == IRFormatType.LINEAR_10MK:
                        ...
                        plt.imshow(image_Temp_Celsius_high,
                        ...
                        plt.imshow(image_Temp_Celsius_low,
                                   cmap='inferno', aspect='auto')
                        plt.colorbar(format='%.2f')
                        ...
                        image_Temp = (
                            B / np.log(R / ((image_Radiance / Emiss / Tau) - K2) + F)) - 273.15
